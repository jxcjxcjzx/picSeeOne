...
public class ParallelIndexing {
    public static void main(String[] args) throws IOException {
        ...
        if (args.length > 0) {
        	...
        	// provide the argument my self
        	File f = new File("addr for picture dir");     // the place for placing pics, the directory name, changed by jxc in 2014.10.4
            System.out.println("Indexing images in " + "addr for picture dir");
            if (f.exists() && f.isDirectory()) passed = true;
        ...
        // the string store the argument or the directory addr passed in
        String passedin = "addr for picture dir";
        
        // use ParallelIndexer to index all photos from args[0] into "index" ... use 6 threads (actually 7 with the I/O thread).
        ParallelIndexer indexer = new ParallelIndexer(6, "index", passedin) {
